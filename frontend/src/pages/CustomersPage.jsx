  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const data = await customersApi.listCustomers(params)
        if (!cancelled) {
          setRows(data)
          setError(null)
        }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (dialog.mode === 'create') {
        ...
        await customersApi.updateCustomer(dialog.id, payload)
      }
      closeDialog()
      reload()
    } catch (err) {
    ...
    <Box>
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
          ...
          <FormControl size="small" sx={{ minWidth: 140 }}>
            ...
            >
              ...
              <MenuItem value="true">{t('customers.active')}</MenuItem>
              <MenuItem value="false">{t('customers.inactive')}</MenuItem>
            </Select>
          </FormControl>
        </Stack>
      </Paper>
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell>{t('customers.contactName')}</TableCell>
      ...
      <Dialog open={dialog.open} onClose={closeDialog} fullWidth maxWidth="sm">
        <Box component="form" onSubmit={handleSubmit}>
          <DialogTitle>
            {dialog.mode === 'create' ? t('customers.createTitle') : t('customers.editTitle')}
          </DialogTitle>
          <DialogContent>
